...
from app.seed import DEMO_STUDENTS
...
def test_seed_demo_is_idempotent():
    ...
    with SessionLocal() as s:
        ...
        # The seeded login accounts plus the fictional cohort — derived, not
        # hardcoded, so adding a tester account cannot break this test.
        assert students == len(DEMO_STUDENTS) + _FICTIONAL_COHORT
        assert len(scoring.leaderboard(s)) == _FICTIONAL_COHORT
